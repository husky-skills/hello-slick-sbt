...
object HelloSlick extends App {
  val db: H2Driver.backend.Database = Database.forConfig("h2mem1")
  try {
    ...
    val f = setupFuture.flatMap { _ =>
      ...
      val allSuppliersAction: DBIO[Seq[Supplier]] =
        suppliers.result

      val combinedAction: DBIO[Seq[Supplier]] =
        insertAndPrintAction >> allSuppliersAction

      val combinedFuture: Future[Seq[Supplier]] =
      ...
      val filterQuery: Query[Coffees, Coffee, Seq] =
      ...
      val sortByPriceQuery: Query[Coffees, Coffee, Seq] =
